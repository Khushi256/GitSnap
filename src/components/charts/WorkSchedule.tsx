export function WorkSchedule({ data }: { data: { weekday: number; weekend: number } }) {
  ...
  if (total === 0) return <p className="text-sm text-muted-foreground flex h-full items-center justify-center">No recent push events.</p>;
    ...
    <div className="flex h-full flex-col justify-center py-4">
      <div className="flex items-center justify-between mb-6">
        <div className="space-y-1">
          <div className="text-3xl font-display font-semibold text-white">{Math.round(weekdayPct * 100)}%</div>
          <div className="text-[10px] uppercase tracking-widest text-indigo-300 font-medium">Weekday</div>
        </div>
        <div className="text-right space-y-1">
          <div className="text-3xl font-display font-semibold text-white">{Math.round(weekendPct * 100)}%</div>
          <div className="text-[10px] uppercase tracking-widest text-cyan-300 font-medium">Weekend</div>
        </div>
      </div>
