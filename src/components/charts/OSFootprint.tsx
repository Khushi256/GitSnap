export function OSFootprint({
  data,
}: {
  data: { personal: number; external: number; totalEvents: number };
}) {
    ...
    <div className="flex h-full flex-col justify-center py-4">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right space-y-1">
          ...
          <div className="text-[10px] uppercase tracking-widest text-violet-300 font-medium">
      ...
      <p className="mt-6 text-xs text-zinc-500 text-center">
        Analyzed <b>{total}</b> recent contributions across <b>{data.totalEvents}</b>{" "}
        {data.totalEvents === 1 ? "event" : "events"} (commits, PRs, issues, etc).
      </p>
    </div>
  );
}
